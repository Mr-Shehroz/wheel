  ...
  return (
    <section className="bg-black text-white py-16 px-4">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {getVisibleTestimonials().map(({ title, subtitle, message, index }) => (
            <div
              key={index}
              className={`group text-white rounded-2xl border border-white p-8 h-[300px] flex flex-col justify-between text-center shadow-lg hover:scale-105 transition-transform duration-300 ${hoverBgClasses[index % hoverBgClasses.length]}`}
            >
              <h4 className="text-lg sm:text-xl md:text-2xl font-semibold mb-4 group-hover:text-black transition-colors">
                {title}
              </h4>
              <h6 className="text-sm sm:text-base md:text-lg font-medium mb-4 text-gray-400 group-hover:text-black transition-colors">
                {subtitle}
              </h6>
              <p className="text-sm sm:text-base md:text-lg group-hover:text-black transition-colors">
                {message}
              </p>
            </div>
          ))}
      ...
      <div className="flex justify-center mt-24">
        <svg className='h-48 w-48' fill='white' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 468.77 468.21">
